...
function updateColor() {
  ...
  // 更新显示值
  document.getElementById('redValue').textContent = '参考值：' + closestColor;
...
function initializeSliders() {
  setRangeBackground(redRange, '#FB6362');
  setRangeBackground(greenRange, '#62AF61');
  setRangeBackground(blueRange, '#6261FA');
...
function loadDefaultImage() {
  ...
  const loadHandler = function() {
    ...
    closestColorName.textContent = '参考值：比色卡编号';
...
function extractColorsFromImage(image) {
  ...
  sortedColors.forEach(([rgb]) => {
    ...
    label.textContent = '参考值：' + colorName;
...
function initializeImageClick() {
  ...
  uploadedImage.addEventListener('click', function(e) {
    ...
    clickedHexValue.textContent = '';
    clickedRgbValue.textContent = '';
    closestColorName.textContent = '参考值：' + closestColor;
    clickedColorCard.style.display = 'block';
...
function showColorImage(colorName) {
    // 移除已存在的容器
    const existingContainer = document.querySelector('.draggable-image-container');
    ...
    // 处理颜色名称
    let formattedName = colorName;
    console.log('步骤1 - 原始名称:', formattedName);

    // 移除开头和结尾的空格
    formattedName = formattedName.trim();
    console.log('步骤2 - 移除首尾空格:', formattedName);

    // 处理 YELLOW-GREEN 和 BLUE-GREEN
    formattedName = formattedName
        .replace(/\s*-\s*/g, '-')  // 标准化连字符
        .replace(/YELLOW\s*-?\s*GREEN/g, 'YELLOW-GREEN')
        .replace(/BLUE\s*-?\s*GREEN/g, 'BLUE-GREEN');
    console.log('步骤3 - 处理特殊颜色名:', formattedName);

    // 移除末尾的变体标识符（A、B、C、D）
    formattedName = formattedName.replace(/\s*[ABCD]\s*$/, '');
    console.log('步骤4 - 移除变体标识:', formattedName);

    // 处理 GROUP 后面的数字，保留 GROUP 前的空格
    formattedName = formattedName.replace(/GROUP\s+(\d+)/g, 'GROUP $1');
    console.log('步骤5 - 处理GROUP数字:', formattedName);

    // 移除多余的空格（保留颜色名称和 GROUP 之间的单个空格）
    formattedName = formattedName.replace(/\s+/g, ' ').trim();
    console.log('步骤6 - 标准化空格:', formattedName);

    // 确保文件名全大写
    formattedName = formattedName.toUpperCase();
    console.log('步骤7 - 最终文件名:', formattedName);
...
// 修改getClosestColorName函数，只返回颜色名称
